...
<template>
  <div class="fallback-view">
    ...
    <section class="actions-playbooks">
      <article class="panel actions">
        <header>
          <p class="eyebrow">应急动作</p>
          <h3>执行进度</h3>
        </header>
      ...
      <article class="panel playbooks">
        <header>
          <p class="eyebrow">应急手册</p>
          <h3>策略库</h3>
        </header>
        <ul>
          <li v-for="play in data.playbooks" :key="play.id">
            <div>
              <p class="title">{{ play.title }}</p>
              <p class="status">{{ play.status }}</p>
            </div>
...
.gradient {
  background: linear-gradient(135deg, rgba(83, 95, 255, 0.65), rgba(11, 16, 32, 0.95));
