...
import com.letsvpn.auth.service.TokenService;
...
@Tag(name = "用户认证", description = "前台用户注册与登录接口") // <--- 添加 Controller 分组 Tag
@RestController
...
public class AuthController {
    ...
    public R<Map<String, Object>> login(
        ...
        try {
            ...
            if (token instanceof String) {
                tokenService.cacheUserSession((String) token, data);
            }
    ...
    @PostMapping("/logout")
    public R<Void> logout(@Valid @org.springframework.web.bind.annotation.RequestBody AdminLogoutRequest request) {
        tokenService.blacklistUserToken(request.getToken());
        return R.success();
    }
}
