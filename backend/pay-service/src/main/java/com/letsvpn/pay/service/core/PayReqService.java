...
//	PayFormService payFormService;
	...
	@Autowired
	PayRateLimitService payRateLimitService;
...
//		}
		...
		// ── merchant_channel_config 校验 ──────────────────────────────
		List<MerchantChannelConfigDTO> configs =
				adminServiceFacade.fetchMerchantChannelConfigs(platformId, "PAYMENT");

		MerchantChannelConfigDTO selectedConfig = null;

		if (channelId != null) {
			// 客户端指定 cid：验证该商户是否启用此通道
			final Long cid = channelId;
			selectedConfig = configs.stream()
					.filter(c -> cid.equals(c.getPayConfigChannelId()) && Integer.valueOf(1).equals(c.getEnabled()))
					.findFirst().orElse(null);
			if (!configs.isEmpty() && selectedConfig == null) {
				// configs 非空说明 admin 可达，但此通道未启用
				validate(true, "该商户未开启此通道 " + cid, 8009, cid + "");
			}
		} else {
			// 未指定 cid：按权重随机选
			List<MerchantChannelConfigDTO> enabledConfigs = configs.stream()
					.filter(c -> Integer.valueOf(1).equals(c.getEnabled()))
					.collect(Collectors.toList());
			if (!configs.isEmpty()) {
				validate(enabledConfigs.isEmpty(), "该商户无可用通道", 8010);
			}
			if (!enabledConfigs.isEmpty()) {
				selectedConfig = weightedRandom(enabledConfigs);
				channelId = selectedConfig.getPayConfigChannelId();
			}
			validate(channelId == null, "cid 不能为空", 8011);
		}

		// 日限额检查
		if (selectedConfig != null && selectedConfig.getDailyLimit() != null
				&& selectedConfig.getDailyLimit().compareTo(BigDecimal.ZERO) > 0) {
			BigDecimal todayAmount = orderInfoMapper.sumTodayAmount(platformId, channelId);
			if (todayAmount.compareTo(selectedConfig.getDailyLimit()) >= 0) {
				adminServiceFacade.disableMerchantChannelConfig(selectedConfig.getId(), platformId, "PAYMENT");
				validate(true, "通道 " + channelId + " 今日限额已用完", 8012, channelId + "");
			}
		}
		// ─────────────────────────────────────────────────────────────
		...
		payRateLimitService.checkChannelTps(payConfigId);
		...
		payRateLimitService.checkMerchantTps(merchantInfo.getAppId());
		...
		payRateLimitService.checkIdempotency(frontId, platformId);
...
//			log.info("{}", maps.keySet());
	...
	/** 按 weight 字段加权随机选取一个通道配置 */
	private MerchantChannelConfigDTO weightedRandom(List<MerchantChannelConfigDTO> list) {
		int total = list.stream().mapToInt(c -> c.getWeight() != null && c.getWeight() > 0 ? c.getWeight() : 1).sum();
		int rand = new Random().nextInt(total);
		int cumulative = 0;
		for (MerchantChannelConfigDTO c : list) {
			cumulative += (c.getWeight() != null && c.getWeight() > 0 ? c.getWeight() : 1);
