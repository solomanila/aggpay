		...
		if (StrUtil.isNotEmpty(successCode) && successCode.equals(success) && order.getStatus() != 8) {// 退款完成状态，不允许上分操作
			...
			if (a > 0) {
				try {
					...
					orderBuildErrorService.add(platformId, payConfigInfo.getPayConfigId(), 0L, "MQ发送失败", orderId, "PAY_SUCCESS MQ发送异常:" + e.getMessage(), "PayNotifyService");
				}
			}
			...
			if (a > 0) {
				if (record.getStatus() != null) {
					try {
						...
						orderBuildErrorService.add(platformId, payConfigInfo.getPayConfigId(), 0L, "MQ发送失败", orderId, "REFUND_UPDATE MQ发送异常:" + e.getMessage(), "PayNotifyService");
					}
				} else {
					log.warn("REFUND_UPDATE: unrecognised refund code '{}', skipping MQ send, orderId={}", refund, orderId);
				}
			}
			return successResult;
		}
